...
signal to_decimalOut: std_logic_vector (11 downto 0); --to decimal output
...
Timer_com: entity work.time(Behavioral)
...
prng_mux: entity work.rng(Behavioral)
            port map(prng_in => prngOut, muxOut => prng_muxOut);
            
to_Decimal_com: entity work.td(Behavioral)
...
stage_mux1: entity work.stage_mux(Behavioral)
     port map(td_in => to_decimalOut, disp_patt_in => disp_muxOut, sel => sel_disp, Dout => stage_muxOut);  
               
ssd_driver_com: entity work.dp_driver(Behavioral)
